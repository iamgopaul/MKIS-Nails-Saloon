...
export async function sendConfirmationEmail(data: BookingInput): Promise<void> {
  ...
  await resend.emails.send({
    from: `MKIS Nails Saloon <${process.env.NEXT_PUBLIC_BUSINESS_EMAIL}>`,
    to: data.email,
...
<body style="margin:0;padding:0;background:#FAF0EB;font-family:Arial,Helvetica,sans-serif;color:#2C1A14;">
  <div style="max-width:560px;margin:40px auto;padding:0 16px;">
    <div style="background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(233,30,140,0.08);">
      ...
      <!-- Header -->
      <div style="background:linear-gradient(135deg,#C9956B,#E8C4AD);padding:40px 40px 32px;text-align:center;">
      ...
      <div style="padding:36px 40px;">
        <p style="font-size:16px;margin:0 0 20px;line-height:1.6;">
          ...
          Thank you for booking with us! We've received your request and will confirm your appointment shortly.
        </p>
        ...
        <p style="font-size:14px;margin:0;">
          📧 <a href="mailto:${process.env.NEXT_PUBLIC_BUSINESS_EMAIL}" style="color:#C9956B;text-decoration:none;">${process.env.NEXT_PUBLIC_BUSINESS_EMAIL}</a><br>
          📞 <a href="tel:+10000000000" style="color:#C9956B;text-decoration:none;">+1 (000) 000-0000</a>
        </p>
      </div>
      ...
      <div style="background:#F0DDD3;padding:20px 40px;text-align:center;">
        <p style="margin:0;font-size:12px;color:#7A5544;">
          © ${new Date().getFullYear()} MKIS Nails Saloon &bull; 123 Beauty Lane, Your City
        </p>
      </div>
    </div>
  </div>
